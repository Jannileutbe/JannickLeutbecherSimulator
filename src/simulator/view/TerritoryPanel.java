...
public class TerritoryPanel extends Region {
    ...
    public void buildPlayingField(Territory territory) {
        ...
        //Bei Linksklick wird Gegenstand platziert, bei Rechtsklick wird Kontextmenü geöffnet
        canvas.setOnMouseClicked(new EventHandler<MouseEvent>() {
            @Override
            public void handle(MouseEvent me) {
                if (me.getButton() == MouseButton.PRIMARY) {
                    viewModel.handleEventPlacingThings(me);
                    buildPlayingField(territory);
                }else if (me.getButton() == MouseButton.SECONDARY){
                    if (viewModel.buildMyContextMenu(TerritoryPanel.this, me) != null) {
                        MyContextMenu myContextMenu = viewModel.buildMyContextMenu(TerritoryPanel.this, me);
                        Node source = (Node) me.getSource();
                        Stage thisStage = (Stage) source.getScene().getWindow();
                        myContextMenu.show(thisStage);
                        buildPlayingField(territory);
                    }
                }
            }
